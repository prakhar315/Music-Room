...
class UserInRoom(APIView):
    def get(self,request,format=None):
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
        print("User In Room:", self,request.session.session_key)
...
class LeaveRoom(APIView):
    def post(self,request,format=None):
        if 'room_code' in self.request.session:
            ...
            current_host = self.request.session.session_key
            room_results = Room.objects.filter(host=current_host)
    ...
    def patch(self,request,format=None):
        ...
        if serializer.is_valid():
            ...
            if user_id != room.host:
                return Response({'msg':'Only host is allowed to change the room settings'},status=status.HTTP_403_FORBIDDEN)
